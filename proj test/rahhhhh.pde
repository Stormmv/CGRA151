...
color startColor, exitColor, settingsColor, scoreboardColor, deadMainMenuColor, easyModeColor, hardModeColor, settingsMainMenuColor;                    //this is to store the color of the buttons
color startHighlight, exitHighlight, settingsHighlight, scoreboardHighlight, deadMainMenuHighlight, easyModeHighlight, hardModeHighlight, settingsMainMenuHighlight;    //this is to store the color of the buttons when the mouse is over them
...
void keyPressed(){  //for checking when user presses a key and changes the angle so you cant be dumb and go backwards
    if (keyCode == UP && angleFacing != 270 && (geckoY[1]-8) != geckoY[2]){
        angleFacing = 90;
    }
    if (keyCode == DOWN && angleFacing != 90 && (geckoY[1]+8) != geckoY[2]){
        angleFacing = 270;
    }
    if (keyCode == LEFT && angleFacing != 0 && (geckoX[1]-8) != geckoX[2]){
        if (easyMode) {
            angleFacing = 180;
        } else {
            angleFacing = 0;
    ...
    if (keyCode == RIGHT && angleFacing != 180 && (geckoX[1]+8) != geckoX[2]){
        if (easyMode) {
            angleFacing = 0;
        } else {
            angleFacing = 180;
...
void move(){
    for (int i = geckoSize; i > 0; i--) {
        if (i != 1) {
            ...
            switch (angleFacing) {
                case 0:
                    geckoX[1] = (geckoX[1] + 8) % width;  // Wrap around horizontally
                    break;
                case 90:
                    geckoY[1] -= 8;
                    break;
                case 180:
                    geckoX[1] = (geckoX[1] - 8 + width) % width;  // Wrap around horizontally
                    break;
                case 270:
                    geckoY[1] += 8;
                    break;
